"""The gating hierarchy, exactly as specified — data-driven so it's trivial to edit.

Each GateSpec is one step. `x`/`y` may be either a detector name ("FSC-A") or a
marker name ("CD45", "Zombie"); markers are resolved to the file's real detector
channel at load time via the session's marker map. Add subpopulation gates
(step #7) just by appending more GateSpecs with the right `parent`.
"""
...
class GateSpec:
    ...
    x: str             # detector or marker for X axis
    y: str             # detector or marker for Y axis
    note: str = ""


# The hierarchy you described, 1:1.
...
GATE_HIERARCHY: list[GateSpec] = [
    ...
    # GateSpec("CD4_T",  "Category_Cells", "CD4",  "CD8", note="subpopulation"),
]
